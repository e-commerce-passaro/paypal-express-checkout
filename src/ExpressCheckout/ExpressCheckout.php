...
use Ecompassaro\Paypal\ExpressCheckout\Request\Payment as PaymentRequest;
use Ecompassaro\Paypal\ExpressCheckout\Result\Set as ResultSet;
...
//         $result->setBuild($BUILD);
    ...
    {
        if (! isset($config['URL'])) {
            throw new Exception(Exception::CONFIG_PARAM_HOST_INVALID);
        }

        if (! isset($config['USER'])) {
            throw new Exception(Exception::CONFIG_PARAM_USER_INVALID);
        }

        if (! isset($config['PWD'])) {
            throw new Exception(Exception::CONFIG_PARAM_PWD_INVALID);
        }

        if (! isset($config['SIGNATURE'])) {
            throw new Exception(Exception::CONFIG_PARAM_SIGNATURE_INVALID);
        }
    }
